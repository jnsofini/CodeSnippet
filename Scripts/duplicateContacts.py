...
def get_duplicates(df, subset=None, full=True): 
    """Takes a dataframe and a set of column names and return (unque) duplicates

    Args:
        df (pd.dataframe): The dataframe tp process
        subset (str or list): column(s) to get duplicates on
        full (bool): Defaults to true so all duplicated are returned, otherwise only unique list of 
...
def get_duplicate_contacts(df, subset=None, unique=False): 
    """Takes a dataframe and a set of column names and return unque duplicates
    ...
    """
    df = df.copy()
    df['NonNanCount'] = df.count(axis=1) #Adds a column of non-nan counts
    df.sort_values(by=list(flatten([subset, 'NonNanCount'])), ascending=False, inplace=True)
    #mask = df.duplicated(subset=subset, keep=False)    
    if unique:
        duplicates = df[df.duplicated(subset=subset, keep=False)].drop_duplicates(subset=subset)
    else:
